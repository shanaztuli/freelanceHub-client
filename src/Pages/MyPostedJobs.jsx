...
const MyPostedJobs = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      fetch(
        `https://freelance-hub-server-ebon.vercel.app/myjobs?email=${user.email}`
      )
        .then((res) => res.json())
  ...
  const handleUpdate = () => {
    fetch(
      `https://freelance-hub-server-ebon.vercel.app/jobs/${updatingJob._id}`,
      {
        method: "PUT",
      ...
      .then((data) => {
        ...
        const updatedJobs = jobs.map((job) =>
          job._id == updatingJob._id ? { ...job, ...updateForm } : job
        );
        setJobs(updatedJobs);
        setUpdatingJob(null);
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://freelance-hub-server-ebon.vercel.app/jobs/${id}?email=${user.email}`,
          { method: "DELETE" }
        )
          .then((res) => res.json())
          .then(() => {
